...
BOOL AppDel_didFinishLaunching(AppDelegate *self, SEL _cmd, id notification) {
	...
	self->window = cmacs_window_init_msgSendCMRect(self->window, sel_getUid("initWithContentRect:styleMask:backing:defer:"), (CMRect){0,0,1024,460}, (NSTitledWindowMask | NSClosableWindowMask | NSResizableWindowMask | NSMiniaturizableWindowMask), 0, false);

	/// Create an instance of our view class.
	///
	/// Relies on the view having declared a constructor that allocates a class pair for it.
	id view = cmacs_rect_msgSendCMRect(cmacs_simple_msgSend((id)objc_getClass("View"), sel_getUid("alloc")), sel_getUid("initWithFrame:"), (CMRect){ 0, 0, 320, 480 });
	
	// here we simply add the view to the window.
	cmacs_void_msgSendVoid(self->window, sel_getUid("setContentView:"), view);
	...
	cmacs_void_msgSendVoid(self->window, sel_getUid("makeKeyAndOrderFront:"), self);
	return YES;
}
...
void RunApplication(void) {
	...
	cmacs_void_msgSendVoid(NSApp, sel_getUid("setDelegate:"), appDelObj);
